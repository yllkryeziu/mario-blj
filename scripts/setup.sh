...
apply_patches() {
  # $1 is the checkout, $2 the patch name prefix. A patch that applies in reverse is already in
  # the tree, which is how re-running setup.sh stays a no-op instead of failing on the second try.
  for patch in "$PATCHES"/"$2"-*.patch; do
    [ -e "$patch" ] || continue
...
apply_patches "$THIRD_PARTY/libsm64" libsm64
apply_patches "$THIRD_PARTY/sm64-port" sm64-port
